...
export default function MaintenanceProfilePage() {
    const { data: authData, isLoading: loadingAuth } = useSWR('/api/auth/me', fetcher);
    const staff = authData?.staff || null;
    ...
    const { data: countsData, isLoading: loadingStats } = useSWR(staff ? `/maintenance/counts${deptParam}` : null, countFetcher);
    const pendingCount = countsData?.pending || 0;
    const resolvedCount = countsData?.resolved || 0;

    if (loadingAuth || (staff && loadingStats)) return <div style={{ display: 'flex', justifyContent: 'center', marginTop: 100 }}><div className="spinner"></div></div>;
